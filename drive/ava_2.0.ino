...
void setup() {
    ...
    stopMotors(); // **Stop motors at startup**
}
    ...
    switch(obsState) {
        ...
        case NORMAL_DRIVING: {
            ...
            if(obstacleFrontCounter >= 3){
                stopMotors();
                // **Quadrant-based turn**
                if(x <= 1.0f) desiredTheta = M_PI/2;   // left side
                else desiredTheta = -M_PI/2;          // right side
                desiredTheta = normalizeAngle(desiredTheta);
                obsState = TURN_TO_PARALLEL;
                break;
            }

            // --- NORMAL FORWARD CORRECTION ---
        ...
        case TURN_TO_PARALLEL: {
            ...
            // Allow slight tolerance (~8.5 deg) to start moving along obstacle sooner
            if(!isThetaAcceptable(err, 0.30)){
                pivotInPlace(err);
                return;
            }
                stopMotors();
                obsState = FOLLOW_OBSTACLE; // start moving along obstacle
        } break;
        ...
        case FOLLOW_OBSTACLE: {
            ...
            // --- Maintain desiredTheta while moving forward ---
            if(!isThetaAcceptable(err, 0.30)){
                pivotInPlace(err);
                return;
            } 
            ...
            // --- SIDE SENSOR CHECK FOR OBSTACLE CLEARANCE ---
